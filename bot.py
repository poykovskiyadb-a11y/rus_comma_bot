...
user_data_lock = threading.Lock()  # Блокировка для потокобезопасного доступа к данным
...
class TelegramBot:
    ...
    def initialize(self):
        ...
        try:
            ...
            if not API_TOKEN:
                raise ValueError("API_TOKEN не найден в config.py")
            ...
            logger.error(f"❌ Не удалось импортировать aiogram: {e}")
            return False
        except Exception as e:
...
"""
        ...
        @self.dp.message(lambda message: message.text == "📖 Правило")
...
"""
        ...
        @self.dp.message(lambda message: message.text == "💪 Работа над ошибками")
        ...
        @self.dp.message(lambda message: message.text == "🧹 Очистить историю ошибок")
        async def clear_mistakes(message: types.Message):
            ...
            with user_data_lock:
                if user_id in user_data:
                    ...
                    save_user_data(user_data)
                    await message.answer("✅ История ошибок очищена!", reply_markup=self._get_main_keyboard())
                else:
                    await message.answer("❌ Ошибка: данные пользователя не найдены", reply_markup=self._get_main_keyboard())
        
        @self.dp.message(lambda message: message.text == "🚀 Начать тест")
...
"""
            await message.answer(question_text, reply_markup=self._get_test_keyboard())
        
        @self.dp.message(lambda message: message.text in ["✅ Да, нужна", "❌ Нет, не нужна"])
        async def check_answer(message: types.Message):
            user_id = str(message.from_user.id)
            
            with user_data_lock:
                if user_id not in user_data or "current_example" not in user_data[user_id]:
                    await message.answer("❌ Сначала начните тест, нажав '🚀 Начать тест'", reply_markup=self._get_main_keyboard())
                    return
...
"""
        ...
        @self.dp.message(lambda message: message.text == "➡️ Следующий вопрос")
        async def next_question(message: types.Message):
            await start_test(message)
        
        @self.dp.message(lambda message: message.text == "🔙 В меню")
        async def back_to_menu(message: types.Message):
            ...
            await message.answer("Возвращаемся в главное меню...", reply_markup=self._get_main_keyboard())
        
        @self.dp.message()
        async def unknown_message(message: types.Message):
            await message.answer("Я не понимаю эту команду. Используйте меню ниже:", reply_markup=self._get_main_keyboard())
    ...
    async def run(self):
        ...
        # Запускаем бота
        await self.dp.start_polling(self.bot, handle_signals=False, skip_updates=True)
...
def main():
    ...
    # 2. Создаем и запускаем Telegram бота в отдельном потоке
